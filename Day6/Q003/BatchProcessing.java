...
public class BatchProcessing {
	public static void main(String args[]) {
		...
		BatchSummary result1 = orderProces.orderprocessing(orders);
		System.out.println(result1.toString());
		BatchSummary result2 = orderProces.orderprocessing(orders);
		System.out.println(result2.toString());
		BatchSummary result3 = orderProces.orderprocessing(orders);
		...
		List<Order> notifyOrders = List.of(
			new Order(1, "payload", "IN", 100),
			new Order(2, "payload", "US", 0),
			new Order(3, "payload", null, 50)
		);
		orderProces.notifyAll(notifyOrders).join().forEach(System.out::println);
	}
